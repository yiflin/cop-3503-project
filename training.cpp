...
std::string Training::sceneloop(std::string start)
{
  Input input;
  int command;
  ...
  while (go)
  {
    ...
    while (line.compare("STOP") != 0)
    {
      ...
      if (lineCount - messageLineCount < 3 || line.find("GAME OVER") != std::string::npos) // there is no next scene or there is only one next scene
      {
        std::getline(file, line);
        currentFrame = line;
        go = false;
